-- Land the `_writer_schema` body edit from commit 6633573 ("allow spaces in
-- defterm names (Malaysia voices)") on every prompt_templates row that still
-- carries the PRE-edit body — including '__shared__' itself, because the
-- forward re-seed 20260605000001 was regenerated in place and prod/dev have
-- already applied it under its old content (supabase runs each migration once
-- by name, so the regenerated body never reaches an existing database).
--
-- Guarded by the OLD body's sha256 so it only rewrites untouched seed/backfill
-- copies: voice rows that an operator localized at runtime (e.g. bowtie-zh-my /
-- bowtie-en-my carry translated bodies with different shas) are NOT clobbered.
-- Idempotent: a second run matches zero rows.
UPDATE content_tool.prompt_templates
SET body = $pt$JSON-LD Schema 規則（去重 / dedup against Yoast）：
...
$pt$,
    sha256 = 'b38d6908983e6dd3a2055280da14edb70b058f42def4c8fccc06888be9e7b834',
    bytes = 2492
WHERE template_id = '_writer_schema'
  AND sha256 = '82bbe0d5d0bcc1a0953a6156ee916a1b47df5fe3d0ef2de371ff02b85fe2d421';
