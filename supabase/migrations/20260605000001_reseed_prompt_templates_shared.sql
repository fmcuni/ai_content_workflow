...
VALUES
  ('__shared__', 'audit', 'agent', 'audit.md', $pt${persona_block}
...
$pt$, '115f41e4973ab288a9759e992991a145fb98f77d2a24ca91fcf70dd228cfcd96', 2003),
  ('__shared__', 'gap_analysis', 'agent', 'gap_analysis.md', $pt$你是香港繁體中文 SEO 內容更新策略助手，專門為現有文章進行 content gap analysis，並判斷應採用 small_refresh 或 full_rewrite 兩條 route 之中的哪一條。
...
$pt$, '12cc09476d909cd4961ecf5f12be7eed07dcb0077996abc12702a06df9c2af6e', 1688),
  ('__shared__', 'topic_dedup', 'agent', 'topic_dedup.md', $pt$你是香港網誌內容研究助理。你的任務是檢查輸入的單一 topic，判斷 site:bowtie.com.hk/blog 是否已經有一篇文章明確寫過相同 topic。
...
$pt$, '3276a614a7b6330194f4d10d20ccc6b1aea8db7a743e5fa41f4bbf90c358f71d', 2473),
  ('__shared__', 'topic_existing_search', 'agent', 'topic_existing_search.md', $pt$你是香港網誌內容檢索助理。你的唯一任務是用 Google 搜尋，找出 site:bowtie.com.hk/blog 上與輸入 topic 最相關的現有文章。
...
$pt$, '0d037f7b39edd41af9813366d1f85dd2db1a1576c05265737f549a3c69625210', 810),
  ('__shared__', 'topic_hot', 'agent', 'topic_hot.md', $pt$你是香港繁中 SEO 研究助理，專門分析 Google SERP 主題熱度。
...
$pt$, '1a8a0e59c5a44f734d79611271950e949f9a1d7a554671463cdd714ebb04544b', 341),
  ('__shared__', '_writer_refine_notes', 'partial', '_writer_refine_notes.md', $pt$如有 refine_notes：
...
$pt$, 'e54e874a733694d022f819c7c040d60b42fbe5e116df3792b0c5446f49c48904', 184),
  ('__shared__', '_writer_output_format_tail', 'partial', '_writer_output_format_tail.md', $pt$2. markup 只可輸出最終完整文章 Markdown，不可輸出任何解說、前言、註解、JSON code fence。
...
$pt$, '3b326aed60a8df47489a7074b9d7cd805f42a72c52874fb901df48de498bc242', 1027),
  ('__shared__', 'judge_brand_voice', 'judge', 'brand_voice.md', $pt$你是品牌語氣審核員。比較以下 final_html 與 persona pack 的 voice_rules、banned_terms、required_phrasings、tone_examples，回傳 1–5 分（1=不符；5=完美）以及具體引用。
...
$pt$, 'b94d20e0814ce5be2eed06c005bc23332a93690826471d379fd5dbb6cd3d9275', 380),
  ('__shared__', 'judge_coverage', 'judge', 'coverage.md', $pt$你是 coverage 審核員。比較 gap_analysis.update_plan 與 final_html，回傳每個 must_* 項目是否被處理。
...
$pt$, '202fc5040defcb473bf1b6a0eacc7870b14870d07a7163fbd7bc6069000d0174', 366),
  ('__shared__', 'judge_citation_alignment', 'judge', 'citation_alignment.md', $pt$你是引用對齊審核員。對每個 citation_intents[i].claim，使用 urlContext 讀取對應 citations[i].final_url 並判斷該頁面是否支持該 claim。
...
$pt$, '556b70ace233514d93b3afeb48cb33e6ed1ecfe19b1a3621f82e6ff3eeddea70', 310),
  ('__shared__', 'judge_hk_localisation', 'judge', 'hk_localisation.md', $pt$你是香港在地化審核員。讀取 final_html，找出任何內地專屬詞彙、非繁體中文、文化不符之處。
...
ON CONFLICT (voice_slug, template_id) DO UPDATE SET
    ...
    updated_at = now(),
    updated_by = 'migration:reseed_prompt_templates_shared';
