-- FROZEN pre-voice baseline -- do NOT regenerate or rewrite this file.
-- This is the original prompt-template store creation: PK (template_id), no
-- voice_slug. It runs BEFORE 20260604172254_per_voice_prompt_library.sql adds
-- voice_slug and repoints the PK to (voice_slug, template_id), so it MUST keep
-- its pre-voice shape or `supabase db reset` breaks ("column already exists" /
-- duplicate PK). It is already applied on prod (supabase runs each migration
-- once by name). scripts/gen_prompt_seed.py NO LONGER writes this file; it now
-- emits the forward re-seed 20260605000001_reseed_prompt_templates_shared.sql,
-- which is how .md body edits propagate going forward.
--
