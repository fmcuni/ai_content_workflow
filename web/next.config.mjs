...
const nextConfig = {
  ...
  async rewrites() {
    ...
    return [
      // SSE ticket issuer (session-protected on the backend). PATH-PRESERVING —
      // keep the `/api` prefix so the same-origin proxy lines up with the backend
      // mount at `/api/auth-ticket`.
