...
@pytest.mark.asyncio
async def test_publish_node_survives_transient_infra_block(db_session):
    """A transient infra block (2xx non-JSON) on the first PUT, then a clean
    JSON success, must still drive the run to status='published' with the
    correct wp_pushed_post_id — the client retries internally and the node
    never sees a raw decode error."""
    run_id = uuid4()
    db_session.add(Run(
        run_id=run_id, created_by="x", status="hitl_2",
        article_url="https://e.com", topic="x", keywords=[], mode="auto",
    ...
    with respx.mock(assert_all_called=True) as r:
        route = r.put("https://wp.example.com/wp-json/wp/v2/posts/98785").mock(
            side_effect=[
                # Infra block: 2xx but HTML body (WAF/CloudFront challenge).
