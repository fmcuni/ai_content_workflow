 ...
 * missing/invalid/expired token this returns the repo's flat 401 body. No DB hop
 * here — session validation is stateless JWT verification.
 */
...
/**
 * Provisioning gate, applied AFTER session validation: resolve the effective
 * role and deny when it is null (no `app_user` row, or `status='disabled'`).
 * This is what makes admin delete/disable take effect on the target's NEXT
 * request — the access token itself is stateless and stays cryptographically
 * valid until expiry (~1h), so without this hop a deleted or disabled user
 * would retain access to every non-role-gated route until their token expired
 * or they logged out.
 *
 * Cost: one indexed `app_user` SELECT per request, cached on the request
 * context — `requireRole`-gated routes reuse it instead of querying again.
 */
 ...
 *
 * - REST: verifies an `Authorization: Bearer <jwt>` Supabase access token.
 * - SSE (paths ending in "/events") and the collab WebSocket (paths ending in
 *   "/doc"): opened cross-origin without the header, so they are authenticated
 *   by the short-lived `?ticket=` HMAC instead (the ticket is minted after this
...
export async function requireAuth(c: AuthContext, next: Next): Promise<Response | void> {
  ...
  // (the browser cannot attach the Bearer header to a WS upgrade), so it is
  // ticket-authed exactly like the SSE `/events` stream. The ticket is
  // HMAC-minted by the backend after a REST session check. Scope the `/doc`
  ...
  const denied = await validateSupabaseSession(c);
